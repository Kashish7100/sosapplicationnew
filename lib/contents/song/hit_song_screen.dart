...
class HitSongListScreen extends StatefulWidget {
  const HitSongListScreen({Key? key}) : super(key: key);

  @override
  State<HitSongListScreen> createState() => _HitSongListScreenState();
}

class _HitSongListScreenState extends State<HitSongListScreen> {
  ...
  void getSongData(searchText) async {
    try {
      http.Response response = await SongServices.getHitSongData(searchText);
      Map responseMap = jsonDecode(response.body);

      if (responseMap['Status'] == true) {
        setState(() {
          songs = responseMap['SongList'];
        });
        print(songs);
      } else {
