...
class SVIP2Screen extends StatefulWidget {
  const SVIP2Screen({Key? key, required this.isSecondPageOpen, required this.toggleFirstPage, required this.toggleSecondPage,
    required this.toggleThirdPage}) : super(key: key);

  final bool isSecondPageOpen;
  final VoidCallback toggleFirstPage;
  final VoidCallback toggleSecondPage;
  final VoidCallback toggleThirdPage;

  @override
  State<SVIP2Screen> createState() => _SVIP2ScreenState();
}

class _SVIP2ScreenState extends State<SVIP2Screen> {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: AnimatedContainer(
        ...
        curve: Curves.linear,
        height: widget.isSecondPageOpen ? MediaQuery.of(context).size.height * 0.7 : 0,
          ...
          image: DecorationImage(
            image: Image.network(
              ...
              errorBuilder: (context, error, stackTrace) {
                return Image.asset('assets/images/shield/SVIP2.png',
                  fit: BoxFit.cover,);
              },
            ).image,
            fit: BoxFit.cover,
